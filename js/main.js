...
getFriendsLink.addEventListener("click", () => {
  ...
  getFriendsLink.setAttribute(
    "class",
    "pure-menu-item friends pure-menu-selected"
  );

  fetch(friendsAll, { method: "GET" })
    .then((response) =>
      // put data into html
      // returns an object
      response.json()
    ...
    .then((responseBodyData) => {
      ...
      singleFriendLink.forEach((link) => {
        link.addEventListener("click", () => {
          ...
          fetch(thisJsonUrl, { method: "GET" })
            .then((response) => response.json())
            .then((responseBodyDataSingle) => {
...
const createFriend = (friends, parentList) => {
  ...
  const thisLink = createElementNew(
    "a",
    friendInfo,
    parentList,
    "pure-menu-link",
...
const createFriendSingle = (oneFriend) => {
  const friendDivParent = createElementNew(
    "div",
    "",
    outputContent,
    "friend",
  ...
  const friendDivInner = createElementNew(
    "div",
    "",
    friendDivParent,
    "identity",
  ...
  const title = createElementNew(
    "h2",
    `${oneFriend.firstName}${oneFriend.lastName}`,
    friendDivInner,
    "name"
  );
  ...
  const fhometown = createElementNew(
    "li",
    `${oneFriend.hometown}`,
    fUl,
    "label"
...
const createFriendList = (friends) => {
  ...
  friends.forEach((value) => {
    const friendListLi = createElementNew(
      "li",
      "",
      friendListUl,
      "pure-menu-item friend-content"
    );
